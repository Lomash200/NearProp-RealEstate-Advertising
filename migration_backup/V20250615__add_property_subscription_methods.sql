...
CREATE INDEX idx_properties_subscription_id ON properties(subscription_id);

-- Add index on owner_id and active status for faster lookups of inactive properties by owner
CREATE INDEX idx_properties_owner_active ON properties(owner_id, active);

-- Add index on subscription_id and expiry date for faster lookups
CREATE INDEX idx_properties_subscription_expiry ON properties(subscription_id, subscription_expiry);

-- Add indexes for faster property lookups by subscription ID and activation status
CREATE INDEX IF NOT EXISTS idx_property_subscription_id ON properties (subscription_id);
...
CREATE INDEX IF NOT EXISTS idx_property_owner_active ON properties (owner_id, active);
